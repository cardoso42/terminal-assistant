...
std::string CurlClient::post(const std::string_view url, const std::string_view data,
                             const std::vector<std::string> &headers)
{
    std::string response_string;
    const CurlPtr curl(curl_easy_init());
    if (!curl)
    ...
    const CurlSlistPtr header_list(raw_headers, &curl_slist_free_all);
    ...
    if (res != CURLE_OK)
    {
        throw std::runtime_error("Erro ao consultar a API: " +
...
size_t CurlClient::write_callback(void *contents, const size_t size, size_t nmemb, void *userp)
{
